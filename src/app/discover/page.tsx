...
type Row = { handle: string; name: string; total: number; privacy: "hidden"|"approx"|"exact" };
export default function Page() {
  ...
  const filtered = useMemo(
    () => data
      .filter(r => r.name.toLowerCase().includes(q.toLowerCase()) || r.handle.includes(q))
  ...
  return (
    <div className="space-y-4 mx-auto max-w-2xl py-8">
      ...
      <div className="flex gap-3">
        <input className="border rounded px-3 py-2 flex-1" placeholder="検索..." value={q} onChange={e=>setQ(e.target.value)}/>
      ...
      <ul className="divide-y bg-white rounded border">
        {filtered.map(r=>(
          <li key={r.handle} className="p-3 flex justify-between">
            <div>
              <div className="font-medium">{r.name} <span className="text-neutral-500">@{r.handle}</span></div>
              <div className="text-sm text-neutral-500">privacy: {r.privacy}</div>
            </div>
            <div className="font-semibold">{Intl.NumberFormat("ja-JP").format(r.total)} 円</div>
          </li>
        ))}
      </ul>
    </div>
