...
export default async function Page() {
  ...
  const { data: lines } = await sb.from("portfolio_lines_ja").select("asset_class_ja,pct").eq("snapshot_id", snap?.snapshot_id);
  ...
  return (
    <div className="grid gap-6 md:grid-cols-2 mx-auto max-w-4xl py-8">
      ...
      <PortfolioPie lines={(lines ?? []).map(l => ({ asset_class: (l as any).asset_class_ja, pct: Number((l as any).pct) }))} />
    </div>
  );
}
